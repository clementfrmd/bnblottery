...
contract AgentRaffleV2 is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public organizerStakes;
    address public stakeToken;
    uint256 public constant ORGANIZER_STAKE = 1e18; // 1 USDm (18 decimals)
    ...
    event OrganizerRegistered(address organizer, uint256 stake);
    event OrganizerUnregistered(address organizer, uint256 stakeReturned);
    event OrganizerSlashed(address organizer, uint256 amount, string reason);
    ...
    constructor(address _treasury, address _stakeToken) Ownable(msg.sender) {
    ...
    function drawWinner(uint256 _raffleId, uint256 _randomNumber, bytes32 _vrfHash) external nonReentrant {
        ...
        // Store VRF hash for verification
    ...
    // --- Self-registration for organizers ---

    /// @notice Stake 1 USDm to become an allowed organizer
    function registerAsOrganizer() external nonReentrant {
        require(!allowedOrganizers[msg.sender], "Already registered");
        IERC20(stakeToken).safeTransferFrom(msg.sender, address(this), ORGANIZER_STAKE);
        allowedOrganizers[msg.sender] = true;
        organizerStakes[msg.sender] = ORGANIZER_STAKE;
        emit OrganizerRegistered(msg.sender, ORGANIZER_STAKE);
    }

    /// @notice Unregister and get stake back (only if no active raffles)
    function unregisterAsOrganizer() external nonReentrant {
        require(allowedOrganizers[msg.sender], "Not registered");
        require(organizerStakes[msg.sender] > 0, "No stake to return");

        // Check no active raffles
        for (uint256 i = 0; i < raffleCounter; i++) {
            Raffle storage r = raffles[i];
            if (r.organizer == msg.sender && r.isActive && !r.isDrawn && !r.isCancelled) {
                revert("Cannot unregister with active raffles");
            }
        }

        uint256 stake = organizerStakes[msg.sender];
        organizerStakes[msg.sender] = 0;
        allowedOrganizers[msg.sender] = false;
        IERC20(stakeToken).safeTransfer(msg.sender, stake);
        emit OrganizerUnregistered(msg.sender, stake);
    }

    /// @notice Owner can slash a bad organizer's stake (sent to treasury)
    function slashOrganizer(address _organizer, string calldata _reason) external onlyOwner nonReentrant {
        uint256 stake = organizerStakes[_organizer];
        require(stake > 0, "No stake to slash");
        organizerStakes[_organizer] = 0;
        allowedOrganizers[_organizer] = false;
        IERC20(stakeToken).safeTransfer(treasury, stake);
        emit OrganizerSlashed(_organizer, stake, _reason);
    }
    ...
    // --- Immediate admin functions (non-sensitive) ---
    ...
    /// @dev Anyone can call this after endTime + 7 days grace period
    ...
    /// @dev Cannot withdraw funds that belong to active/uncancelled raffles
    function emergencyWithdraw(address _token, uint256 _amount) external onlyOwner nonReentrant {
        require(_token != address(0), "Invalid token");

        // Calculate total funds locked in active raffles for this token
        uint256 lockedFunds = 0;
        for (uint256 i = 0; i < raffleCounter; i++) {
            Raffle storage r = raffles[i];
            if (r.paymentToken == _token && !r.isDrawn && r.totalTickets > 0) {
                // Count unrefunded tickets (active raffles + cancelled awaiting refunds)
